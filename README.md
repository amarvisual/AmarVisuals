...
- **2026-08-12**: 6 Selected Projects & Admin Image Uploader — Expanded the Recent Projects portfolio grid in `index.html` from 3 to 6 projects with bespoke screenshots (*OdiaTyping.in, Kashvi Speech Therapy, Amar Visual Studio, Utkal Dental Clinic, Kalinga Fresh Organic Store, Bhubaneswar Tech Hub*). Updated the Admin Control Panel (`admin.html` and `admin/index.html`) to provide individual controls for all 6 projects including live preview thumbnails, image file pickers, and URL setters.
- **2026-08-12**: Real Project Integration (Kashvi Speech Therapy) — Integrated live clinical project *Kashvi Speech Therapy & Hearing Clinic* (`https://kashvi-speech-therapy.vercel.app/`) with high-resolution screenshot `kashvi.png`, direct outward navigation, and synchronized Admin Panel defaults.
